...
object WikipediaRanking extends WikipediaRankingInterface {
  ...
  def occurrencesOfLang(lang: String, rdd: RDD[WikipediaArticle]): Int = rdd
    .filter(article => new WikipediaArticle(article.title, article.text).mentionsLanguage(lang)).count().toInt
  ...
  def rankLangsUsingIndex(index: RDD[(String, Iterable[WikipediaArticle])]): List[(String, Int)] = {
    index.map(pair => (pair._1, pair._2.size)).sortBy(_._2, false).collect().toList
  }
